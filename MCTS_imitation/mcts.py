...
class MonteCarloTreeSearch(object):
    ...
    def playout(self, node):
        ...
        if node.game_status is RESULTS.NOT_FINISHED:
            ...
            for move in moves:
                ...
                if is_done:
                    game_status = RESULTS.WON if node_current.env.PLAYER is PLAYER.FIRST else RESULTS.LOST
    ...
    def train_mcts_online(self,node, training_time):
        ...
        while (current - start) < training_time and counter < 20:
            self.mcts_one_step(self.root)
            current = int(round(time.time() * 1000))
            counter += 1
        
        print("Total number of steps : ",counter)
        return self.root
